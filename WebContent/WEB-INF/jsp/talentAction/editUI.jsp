...
<body>
	...
	<!--显示表单内容-->
	<div id=MainArea>
		<s:form action="talent_edit.action" method="post"
				...
				<div class="ItemBlock">
					<table class="table table-bordered">
						...
						<tr>
							...
							/>
							<p id="talent_edit_action_name_info" class="muted"></p>
							<td>邮&nbsp;&nbsp;箱: <s:textfield name="email"
									cssClass="InputStyle" />
							</td>
							<td rowspan="8"><label class="control-label">${ photoUrl}</label>
							<s:file name="photo"></s:file></td>
						</tr>
						<tr>
						...
						<tr>
							...
							<p id="talent_edit_action_birthday_info" class="muted"></p>
						...
						<tr>
							<td><label class="muted">*</label>性别: <s:radio list="{'男','女'}" name="sex" />
							<td><label class="muted">*</label>联系电话: <s:textfield name="phoneNumber" onBlur="phoneNumber_validate();"/>
							<p id="talent_edit_action_phoneNumber_info" class="muted"></p>
							</td>
						</tr>
						<tr>
							<td><label class="muted">*</label>第一学历: <s:select headerKey="" headerValue="请选择"
									name="firstDegree"
						...
						<tr>
							...
							/>
							<p id="talent_edit_action_position_info" class="muted"></p>
							</td>
						</tr>
						<tr>
						...
						<tr>
							...
							<table class="table table-bordered">
						<thead>
							<th colspan="3">工作经历 <!-- <input type="button" value="增加"
								onClick="addMore()" /> -->
							</th>
						</thead>
						<tr>
							...
							<td><h4>所在职位</h4></td>
								...
								<tr id="${id }">
									<td><input type='text' name='startDate'
										value="${startDate }" />至 <input type='text' name='endDate'
										value="${ endDate}" /></td>
									<td><input type='text' name='company' 
										value="${company }" /></td>
									<td><input type='text' name='job'
										class='delete' value="${job }" /></td>
								</tr>
								<tr>
							<td colspan="3"><h4>岗位职责</h4></td>
							</tr>
								<tr id="${id }">
								<td colspan="3"><s:textarea name="duty" rows="3" cols="150"></s:textarea></td>
								</tr>
								<tr>
							<td colspan="3"><h4>工作业绩</h4></td>
							</tr>
								<tr id="${id }">
								<td colspan="3"><s:textarea name="kpi"  rows='3' cols='150'></s:textarea>
								&nbsp<input
										type="button" value="删除" onclick="del_tr(${id});" /></td>
								</tr>
							</s:iterator>
						</tbody> --%>
						</table>
						...
						<tr>
							<td colspan="1"><label class="control-label">${oldResumeName }</label>
							<s:file name="oldResume"></s:file></td>
							<td colspan="2"><s:file name="standardResume">
									<label class="control-label">${standardResumeName }</label>
								</s:file></td>
						</tr>
						<thead>
...
function addMore() {
	...
	function name_validate(){
		var name = $.trim( $('#talent_edit_action_name').val() );
		if( name == '' ){
			$('#talent_edit_action_name').val('');
			return change_validate_style('toError','#talent_edit_action_name','请填写姓名');
		} else {
			return change_validate_style('toSuccess','#talent_edit_action_name','');
		}
	}
	
	function birthday_validate(){
		var birthday = $.trim( $('#talent_edit_action_birthday').val() );
		if( birthday == '' ){
			$('#talent_edit_action_birthday').val('');
			return change_validate_style('toError','#talent_edit_action_birthday','请填写出生年月');
		} else {
			return change_validate_style('toSuccess','#talent_edit_action_birthday','');
		}
	}
	
	function phoneNumber_validate(){
		var phoneNumber = $.trim( $('#talent_edit_action_phoneNumber').val() );
		if( phoneNumber == '' ){
			$('#talent_edit_action_phoneNumber').val('');
			return change_validate_style('toError','#talent_edit_action_phoneNumber','请填写联系电话');
		} else {
			return change_validate_style('toSuccess','#talent_edit_action_phoneNumber','');
		}
	}
	
	function position_validate(){
		var position = $.trim( $('#talent_edit_action_position').val() );
		if( position == '' ){
			$('#talent_edit_action_position').val('');
			return change_validate_style('toError','#talent_edit_action_position','请填写现任职位');
		} else {
			return change_validate_style('toSuccess','#talent_edit_action_position','');
		}
	}
